...
// Best-effort failure alert — emails the user when this cron fails. Runs every 15 min,
// so throttled to one email per hour via public.should_alert.
async function notifyFailure(errMsg: string): Promise<void> {
  try {
    const sb = getServiceClient()
    const { data: ok } = await sb.rpc("should_alert", { p_fn: "ms-outlook-folders", p_min: "1 hour" })
    if (ok !== true) return
    ...
    if (!email) return
    const html = `<!doctype html><html><body style="font-family:-apple-system,sans-serif;max-width:640px;margin:0 auto;padding:24px;color:#1f2937"><h2 style="color:#b91c1c;border-bottom:2px solid #b91c1c;padding-bottom:8px">ms-outlook-folders failed</h2><p><strong>When:</strong> ${new Date().toISOString()}</p><pre style="background:#fee2e2;padding:12px;border-radius:4px;white-space:pre-wrap;word-break:break-word;font-size:13px">${errMsg}</pre><p style="color:#475569;font-size:13px">Check the Supabase Edge Function logs (ms-outlook-folders). Alerts are throttled to one per hour.</p></body></html>`
    await fetch(`${GRAPH_BASE}/me/sendMail`, { method: "POST", headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" }, body: JSON.stringify({ message: { subject: "⚠️ ms-outlook-folders failed", body: { contentType: "HTML", content: html }, toRecipients: [{ emailAddress: { address: email } }], importance: "high" }, saveToSentItems: "true" }) })
  } catch (alertErr) {
    console.error("ms-outlook-folders failure-alert failed:", alertErr instanceof Error ? alertErr.message : String(alertErr))
  }
}
